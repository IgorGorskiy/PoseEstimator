...
{
    // n+1 = 7 вершин в 6D
    std::vector<PoseVec> simplex(7, x0);
    for (int i = 0; i < 6; ++i) {
    ...
    double alpha, double gamma, double rho, double sigma) const
{
    ...
    for (int i = 0; i <= n; ++i) fval[i] = f(simplex[i]);
                        ...
                        const ImagePreprocessor::Result& prep) const
{
    ...
    auto scoreFn = [this, &prep](const PoseVec& v) -> double {
        float vr;
        double s = matcher_.score(poseVecToSE3(v), prep, &vr);
    ...
    if (!multithread) {
        ...
        double sigma = 0.5;   // shrink
        OptResult result;
        PoseOptimizer::nelderMead(result, x0, scoreFn, alpha, gamma, rho, sigma);
        return result;
    }
