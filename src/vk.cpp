...
namespace VK {
    ...
    void init() {
        ...
        // image color and depth
        {
            ...
            const VkDeviceSize readSize = static_cast<VkDeviceSize>(width) * height * 1; // R8
    ...
    cv::Mat draw(float cam_x, float cam_y, float cam_z, float rot_x, float rot_y, float rot_z) {
        ...
        mats[0] = glm::perspectiveZO(glm::radians(45.f), float(width) / float(height), 0.1f, 200.0f);
        ...
        return cv::Mat(int(height), int(width), CV_8UC1, (void*)readPtr);
    }
